...
- Long-term archival (90-day retention; signers keep their own copy).
...
- **Internal cron endpoint** `/api/internal/cron/` — expiry sweep + 90-day retention deletion. Invoked by Vercel Cron.
...
The certificate is what gives the SES its bevisverdi after we delete the database row (90 days). Every signer receives the final PDF by email, so each party holds their own durable copy.
...
4. **Completion** — all signers in `signed` status:
   - Generate final PDF (original + signature-certificate appendix) via `pdf-lib`.
   - Store as `final_signed_pdf_blob_url`.
   - Email each signer + sender with the final PDF attached. Email body notes: "this document will be deleted from our servers in 90 days — keep this copy."
   - Set `signing_requests.status = completed`.
   - Fire webhook if configured (best-effort, HMAC-signed).
...
7. **Retention sweep** — Vercel Cron daily:
   - Find requests where `(completed_at | cancelled_at | expired_at) < now() - 90 days`. Delete blobs, then delete signing_request row (cascades to documents, signers, audit_events).
...
GET    /documents/:id/final                         Download final PDF (X-Lookup-Token or X-Sign-Token)
```
...
| `download_signed_document` | `signing_request_id`, `sender_lookup_token` | `{filename, content_base64}` |
...
/app
  ...
  /api/v1
    ...
    documents/[id]/final/route.ts            # GET
  /api/internal/cron/
    expire/route.ts
    retention/route.ts
/lib
...
**E2E (Playwright):**
- Happy path: create with 2 signers → sender confirms → signer 1 signs (no SMS) → signer 2 signs (with SMS) → final PDF downloaded, SHA-256 of original section matches `rendered_pdf_sha256`.
...
- Expiry: time-traveled cron run flips status.
- Retention: time-traveled cron run hard-deletes data.
...
- Happy-path E2E passes against deployed staging.
